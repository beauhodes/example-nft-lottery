...
//======================================== Notes ========================================
// This is terribly gas inefficient atm
// Getters need work
// Look at packing/reworking SaleInfo struct
//      And, perhaps remove dynamic arrays in favor of another setup (mapping-focused)
...
contract Lottery is IERC721Receiver, VRFConsumerBase, Ownable {
    ...
    */
    struct SaleInfo { //working on packing - perhaps u8, boolx2, addrx3, dynamic arrays
        address nftContract; //20 bytes
        uint256 tokenId; //32 bytes
        address sellerAddress; //20 bytes
        uint256 depositAmount; //32 bytes
        uint256 currentAmount; //32 bytes
        uint256 maxAmount; //32 bytes
        bool sellerWithdrawn; //1 byte
        bool ownerWithdrawn; //1 byte
        uint8 state; //1 byte
        address winner; //20 bytes
        uint256 expirationBlocks; //32 bytes
        uint256 lastDepositBlock; //32 bytes
        address[] depositors; //dynamic array of 20 bytes per
        bool[] depositsWithdrawn; //dynamic array of 1 byte per
    }

    //packing:
    //32 bytes = 1 slot
    //to pack: 1 uint8, 2 bools, 3 addresses
    //  we want: uint8, bool, bool, address, address, address
    ...
    */
    ...
    //get last randomness request id
    ...
    function withdrawSeller(uint256 _saleId) public {
        ...
        IERC20(base).transfer(msg.sender, withdrawAmount); //todo: check for ret val
